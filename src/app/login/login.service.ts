...
export class LoginService {
  ...
  doLogin(username: string, password: string, userWarehouseId) {
    return new Promise((resolve, reject) => {
      this.http.post(`${this.loginUrl}/login`, { username: username, password: password, userWarehouseId: userWarehouseId, supportLoginSteps: true })
        .map(res => res.json())
        .subscribe(data => {
          resolve(data);
        }, error => {
  ...
  private postWithPreAuth(path: string, preAuthToken: string, body: any = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.http.post(`${this.loginUrl}${path}`, body, { headers: headers })
        .map(res => res.json())
        .subscribe(data => {
          resolve(data);
        }, error => {
